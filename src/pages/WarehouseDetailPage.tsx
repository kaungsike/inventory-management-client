...
export default function WarehouseDetailPage() {
  ...
  const managerItems = Object.fromEntries(
    managers.map((m) => [String(m.id), m.name])
  )
  ...
  const { data: inventoryData, isLoading: inventoryLoading } = useInventory(
    tab === "inventory"
      ? { warehouse_id: warehouseId, page: inventoryPage }
      : { warehouse_id: warehouseId }
  )
  const { data: txData, isLoading: txLoading } = useTransactions(
    tab === "transactions"
      ? { warehouse_id: warehouseId, page: txPage }
      : { warehouse_id: warehouseId, per_page: 1 }
  )
  const { data: poData, isLoading: poLoading } = usePurchaseOrders(
    tab === "purchase-orders"
      ? { warehouse_id: warehouseId, page: poPage }
      : { warehouse_id: warehouseId, per_page: 1 }
  )
  const { data: soData, isLoading: soLoading } = useSalesOrders(
    tab === "sales-orders"
      ? { warehouse_id: warehouseId, page: soPage }
      : { warehouse_id: warehouseId, per_page: 1 }
  )
  ...
  useEffect(() => {
    if (detail)
      setAssignManagerId(detail.manager_id ? String(detail.manager_id) : "")
  }, [detail?.manager_id])
  ...
  const selectedInv = detail.inventory.find(
    (inv) => String(inv.id) === selectedProduct
  )
  ...
  const onTransfer = async (d: TransferFormData) => {
    const inventory = detail.inventory.find(
      (inv) => String(inv.id) === d.product_id
    )
  ...
  const inventoryColumns: ColumnDef<Inventory, unknown>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) =>
        formatCurrency(
          row.original.quantity * (row.original.product?.average_cost ?? 0)
        ),
    },
    ...(!isArchived
      ? [
  ...
  const transactionColumns: ColumnDef<InventoryTransaction, unknown>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <span
          className={
            row.original.quantity > 0 ? "text-green-600" : "text-destructive"
          }
        >
    ...
    },
    {
      ...
      header: "Unit Cost",
      cell: ({ row }) =>
        row.original.unit_cost ? formatCurrency(row.original.unit_cost) : "—",
    },
  ]
  ...
  const soColumns: ColumnDef<SalesOrder, unknown>[] = [
    {
      ...
      header: "SO Number",
      cell: ({ row }) => (
        <span className="font-mono text-xs">{row.original.so_number}</span>
      ),
    },
    {
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-5">
            ...
            <Card>
              <CardContent className="pt-6">
                ...
                <p className="mt-1 text-2xl font-semibold">
                  {formatCurrency(detail.inventory_value)}
                </p>
              </CardContent>
            </Card>
          </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <StatusBadge
                    status={isArchived ? "archived" : detail.status}
                  />
                </div>
                <div className="flex justify-between">
              ...
              <CardContent className="space-y-3">
                  ...
                  <>
                    <p className="text-sm text-muted-foreground">
                      ...
                      <span className="font-medium text-foreground">
                        {detail.manager?.name ?? "Unassigned"}
                      </span>
                    </p>
                    <div className="flex items-center gap-2">
                      <Select
                        value={assignManagerId}
                        onValueChange={(v) => setAssignManagerId(v ?? "")}
                        items={managerItems}
                      >
                ...
                {isArchived && (
                  <p className="text-xs text-muted-foreground">
                    This warehouse is archived. Restore it to resume operations.
                  </p>
                )}
              </CardContent>
            </Card>
      ...
      {tab === "inventory" && (
        <>
          {inventoryLoading ? (
            ...
            <EmptyState icon={Boxes} title="No stock in this warehouse" />
          ) : (
            <>
              ...
              {inventoryData.meta && (
                <TablePagination
                  ...
                  lastPage={inventoryData.meta.last_page}
      ...
      {tab === "transactions" && (
        <>
          {txLoading ? (
            ...
            <EmptyState
              icon={Boxes}
              title="No transactions for this warehouse"
            />
          ) : (
            <>
      ...
      {tab === "purchase-orders" && (
        <>
          {poLoading ? (
            ...
            <EmptyState
              icon={Boxes}
              title="No purchase orders for this warehouse"
            />
          ) : (
            <>
              ...
              {poData.meta && (
                <TablePagination
                  currentPage={poData.meta.current_page}
                  lastPage={poData.meta.last_page}
                  total={poData.meta.total}
      ...
      {tab === "sales-orders" && (
        <>
          {soLoading ? (
            ...
            <EmptyState
              icon={Boxes}
              title="No sales orders for this warehouse"
            />
          ) : (
            <>
              ...
              {soData.meta && (
                <TablePagination
                  currentPage={soData.meta.current_page}
                  lastPage={soData.meta.last_page}
                  total={soData.meta.total}
      ...
      {tab === "transfers" && (
        <>
          {transferLoading ? (
            ...
            <EmptyState
              icon={Boxes}
              title="No transfers involving this warehouse"
            />
          ) : (
            <>
              <DataTable
              ...
              {transferData.meta && (
                <TablePagination
                  currentPage={transferData.meta.current_page}
                  lastPage={transferData.meta.last_page}
                  total={transferData.meta.total}
      ...
      {showTransfer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative mx-4 w-full max-w-md rounded-xl border border-border bg-card p-6 shadow-lg">
            <h2 className="mb-1 text-lg font-semibold">Transfer Stock Out</h2>
            <p className="mb-4 text-sm text-muted-foreground">
              From {detail.name}
            </p>
            <form
            ...
            >
              <div>
                ...
                {transferForm.formState.errors.product_id && (
                  <p className="mt-1 text-xs text-destructive">
                    {transferForm.formState.errors.product_id.message}
                  </p>
                )}
              </div>
              <div>
                ...
                {transferForm.formState.errors.to_warehouse_id && (
                  <p className="mt-1 text-xs text-destructive">
                    {transferForm.formState.errors.to_warehouse_id.message}
                  </p>
                )}
              </div>
              <div>
                  ...
                  {...transferForm.register("quantity", {
                    ...
                    max:
                      availableQty > 0
                        ? {
                            value: availableQty,
                            message: `Max available: ${availableQty}`,
                          }
                        : undefined,
                  })}
                  className="mt-1"
